"""
Everything through --repo_env
VENDOR is optional (and global)
TOOLCHAIN is optional (and global)

 toolchain
 ├── impl
 │   ├── BUILD
 │   ├── toolchain_impl.bzl (formerly toolchain_common.bzl)
 │   ├── build-toolchain.sh
 │   ├── install-toolchain.sh
 │   └── package-toolchains.sh
 ├── BUILD
 ├── initialization.bzl
 ├── toolchain_flags.bzl  (contains flags)
 └── toolchain_config.bzl (contains Manifest.text and URLs)    
"""
...
def determine_toolchain(repo_ctx):
    ...
    # Filter the manifest
    filtered = [x for x in manifest
                if x[1].startswith(toolchain)
                and x[2] == cpu
                and (not vendor or x[3] == "" or x[3] == vendor)
                and x[4] == os]
...
def file_sha256(repo_ctx, filename):
    ...
    if ret.return_code != 0:
        fail("Failed to extract hash, cmd: sha256sum {}, exitcode: {}, stderr: {}",
             bash_quote(filename), ret.return_code, ret.stderr)
...
def download_and_extract_one(repo_ctx, url, archive_name, expected_hash,
                             destination_dir, download_cache_dir):
...
def download_and_extract(repo_ctx, urls, archive_name, expected_hash,
                         destination_dir, download_cache_dir):
    ...
    if not is_extracted(repo_ctx, archive_name, expected_hash, destination_dir):
        for url in urls:
            if download_and_extract_one(repo_ctx, url, archive_name, expected_hash,
                                        destination_dir, download_cache_dir):
                break
...
def initialize_toolchain(urls, manifest, toolchain_flags = {}, no_check_certificate = False):
    initialization(
    ...
    native.register_toolchains("@initialize_toolchain//:cc")
